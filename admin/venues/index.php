    ...
    if (isset($_POST['action'])) {
        ...
        switch ($_POST['action']) {
            case 'toggle_featured':
                ...
                $sql = "UPDATE halls SET is_featured = ? WHERE id = ?";
                $stmt = $db->getConnection()->prepare($sql);
                $stmt->bind_param("ii", $featured, $venueId);
                if (!$stmt->execute()) {
...
$(document).ready(function() {
    ...
    $("#venueSearch").on("keyup", function() {
        ...
        $(".venues-table tbody tr").filter(function() {
    ...
    $("#amenityFilter").on("change", function() {
        var amenity = $(this).val().toLowerCase();
        $(".venues-table tbody tr").filter(function() {
            var amenities = $(this).data("amenities").toLowerCase();
            $(this).toggle(amenity === "" || amenities.indexOf(amenity) > -1);
        });
    });
